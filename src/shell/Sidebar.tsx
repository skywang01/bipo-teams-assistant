  ...
  return (
    <aside className={`sidebar ${collapsed ? "collapsed" : ""}`}>
      <div className="side-top">
        {!inTeams && !collapsed && (
          <div className="side-brand"><span className="side-logo">B</span><span className="side-name">{t("appName")}</span></div>
        )}
        <button className="side-collapse" onClick={toggleCollapsed} aria-label="toggle" title="⟨⟩">
          {collapsed ? "»" : "«"}
        </button>
      </div>
